...
namespace Schedule {
    namespace Internal {
        ...
        void TimerAdd(cpu_t* cpu, thread_t* t, uint64_t expires, uint64_t now) {
            ...
            t->timer_wakeup = expires; t->timer_next = nullptr; t->timer_prev = nullptr; t->timer_cpu = cpu->id;
            thread_t** bucket;
            if (delta < TV_SIZE) bucket = &cpu->tv1[expires & TV_MASK];
            else if (delta < (1ULL << (2 * TV_BITS))) bucket = &cpu->tv2[(expires >> TV_BITS) & TV_MASK];
            else bucket = &cpu->tv3[(expires >> (2 * TV_BITS)) & TV_MASK];
            if (*bucket) (*bucket)->timer_prev = t;
            t->timer_next = *bucket; t->timer_bucket = bucket; *bucket = t;
        }

        void TimerCascade(cpu_t* cpu, thread_t** tv, int idx, uint64_t now) {
            thread_t* t = tv[idx];
            tv[idx] = nullptr;
            while (t) {
                thread_t* next = t->timer_next;
                if (t->timer_wakeup <= now) WakeupTimerThread(cpu, t);
                else {
                    t->timer_bucket = nullptr; t->timer_next = nullptr; t->timer_prev = nullptr;
    ...
    void Sleep(uint64_t ms){
        ...
        while (ms > 0) {
            uint64_t chunk = ms;
            if (chunk >= TIMER_MAX_TIMEOUT_MS) chunk = TIMER_MAX_TIMEOUT_MS - 1;
            ...
            if (expires > PIT::TimeSinceBootMS()) Schedule::Yield();
            ms -= chunk;
        }
    }
    ...
    void Tick() {
        ...
        while (cpu->timer_last_tick < now) {
            ...
            if (idx1 == 0) {
                ...
                Schedule::Internal::TimerCascade(cpu, cpu->tv2, idx2, tick);
                if (idx2 == 0) {
                    int idx3 = (tick >> (2 * TV_BITS)) & TV_MASK;
                    Schedule::Internal::TimerCascade(cpu, cpu->tv3, idx3, tick);
                }
            }
        }
        spinlock_unlock(&cpu->sched_lock);
