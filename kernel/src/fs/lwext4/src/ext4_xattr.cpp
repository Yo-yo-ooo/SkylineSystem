	...
	*found = false;
	...
	for (i = 0; prefix_tbl[i].prefix; i++) {
		...
		if (full_name_len >= prefix_len &&
		    !_memcmp(full_name, prefix_tbl[i].prefix, prefix_len)) {
			bool require_name =
				...
				struct ext4_xattr_search *s, bool dry_run)
{
	...
	if (!s->not_found) {
		...
		if (value_offs) {
			...
			_memmove((char *)first_value + value_size, first_value,
				(char *)value - (char *)first_value);

			/* Zero the gap created */
			_memset(first_value, 0, value_size);
	...
	if (i->value) {
		...
		if (value_offs) {
			...
			if (EXT4_XATTR_SIZE(i->value_len) != i->value_len)
				_memset((char *)s->base + value_offs +
					   i->value_len,
		...
		_memmove(s->here, EXT4_XATTR_NEXT(s->here),
			(char *)last + sizeof(uint32_t) -
			    (char *)EXT4_XATTR_NEXT(s->here));

		/* Zero the gap created */
		_memset((char *)last - shift_offs + sizeof(uint32_t), 0,
		       shift_offs);
...
static void ext4_xattr_ibody_initialize(struct ext4_inode_ref *inode_ref)
{
	...
	_memset(header, 0, inode_size - EXT4_GOOD_OLD_INODE_SIZE - extra_isize);
					...
					struct ext4_block *block)
{
	...
	_memset(block->data, 0, ext4_sb_get_block_size(&fs->sb));
				       ...
				       struct ext4_block *block)
{
	...
	_memset(&finder->s, 0, sizeof(finder->s));
