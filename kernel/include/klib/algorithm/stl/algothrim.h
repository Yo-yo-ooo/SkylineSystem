...
#include <klib/algorithm/stl/typetraits.h>
#include <klib/klib.h>
namespace EasySTL {
    //***** fill O(N)******
    ...
    inline void fill(char *first, char *last, const char& value) {
    	_memset(first, static_cast<unsigned char>(value), last - first);
    }

    inline void fill(wchar_t *first, wchar_t *last, const wchar_t& value) {
    	_memset(first, static_cast<unsigned char>(value), (last - first) * sizeof(wchar_t));
    }
	...
	char *fill_n(char *first, Size n, const char& value) {
		_memset(first, static_cast<unsigned char>(value), n);
		return first + n;
	}

	template<class Size>
	wchar_t *fill_n(wchar_t *first, Size n, const wchar_t& value) {
		_memset(first, static_cast<unsigned char>(value), n * sizeof(wchar_t));
		return first + n;
	}
...
//将最大值放到末尾
 ...
 inline void make_heap(RandomIterator first, RandomIterator last) {
    ...
    OutputIterator __copy(InputIterator first, InputIterator last, OutputIterator result, _true_type){
        ...
        __memcpy(result, first, sizeof(*first) * dist);
        advance(result, dist);
        return result;
