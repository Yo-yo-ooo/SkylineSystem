...
//---------------------------------------------------------------------
    ...
    #define INLINE         __inline__ __attribute__((always_inline)) __attribute__((target("sse2")))
#else
    #define INLINE         __inline__ __attribute__((target("sse2")))
#endif
#elif defined(_MSC_VER)
	#define INLINE __forceinline __attribute__((target("sse2")))
...
//---------------------------------------------------------------------
static INLINE void *memcpy_tiny_sse2(void *dst, const void *src, size_t size) {
...
// main routine
//---------------------------------------------------------------------
static void* memcpy_fast_sse2(void *destination, const void *source, size_t size)
{
	...
	// small memory copy
	if (size <= 128) {
		return memcpy_tiny_sse2(dst, src, size);
	}
	...
	memcpy_tiny_sse2(dst, src, size);

	return destination;
}

static INLINE void *memcpy_tiny_avx(void *dst, const void *src, size_t size) {
	unsigned char *dd = ((unsigned char*)dst) + size;
	const unsigned char *ss = ((const unsigned char*)src) + size;
...
// main routine
//---------------------------------------------------------------------
static void* memcpy_fast_avx(void *destination, const void *source, size_t size)
{
	unsigned char *dst = (unsigned char*)destination;
	const unsigned char *src = (const unsigned char*)source;
