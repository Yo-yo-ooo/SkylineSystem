...
static int memcmp_eq (const void *str1, const void *str2, size_t count)
{
  const unsigned char *s1 = (unsigned char *)str1;
  const unsigned char *s2 = (unsigned char *)str2;
...
//
// This file provides a highly optimized version of memcmp_fpx86.
...
// of their respective sizes, as opposed to the first byte (it seems memcmp_fpx86(3)
...
// memcmp_fpx86 (~5 cycles). It's the mask operations that take ~3 cycles each...
//
// When the latency of jumps are taken into account, that means this function can
// compare 64 BYTES of data at around the same speed that memcmp_fpx86 does only 1 byte.
// The AVX2 version is 1 cycle slower than the AVX512 version in its main loop
// (i.e. it takes ~11 cycles). When an inequality is found, memcmp_fpx86 takes 3 cycles,
...
// memcmp_fpx86 for large chunks of memory with arbitrary sizes
static int memcmp_large(const void *str1, const void *str2, size_t numbytes) // Worst-case scenario: 127 bytes.
{
  ...
  // At minimum non-trivial case, once. Each memcmp_fpx86 has its own loop.
  {
    if(numbytes < 2) // 1 byte
    {
      returnval = memcmp_fpx86(str1, str2, numbytes);
      if(returnval)
      {
        return returnval;
      }
...
// Equality-only version
static int memcmp_large_eq(const void *str1, const void *str2, size_t numbytes) // Worst-case scenario: 127 bytes.
{
  int returnval = 0; // Return value if equal... or numbytes is 0
  size_t offset = 0;
...
// memcmp_fpx86 for large chunks of memory with arbitrary sizes (aligned)
static int memcmp_large_a(const void *str1, const void *str2, size_t numbytes) // Worst-case scenario: 127 bytes.
{
  int returnval = 0; // Return value if equal... or numbytes is 0
  size_t offset = 0;
...
static int memcmp_large_eq_a(const void *str1, const void *str2, size_t numbytes) // Worst-case scenario: 127 bytes.
{
  int returnval = 0; // Return value if equal... or numbytes is 0
  size_t offset = 0;
...
int x86memlib_DeclFunction(AVX_memcmp)(const void *str1, const void *str2, size_t numbytes, int equality)
{
  ...
  {
    // See memmove.c for why it's worth doing special aligned versions of memcmp_fpx86, which
...
int AVX_memcmpV3(const void *str1, const void *str2, size_t numbytes, int equality) {return 0;}
#else
__attribute__((weak))
...
__attribute__((weak, used))
int AVX_memcmpV2(const void *str1, const void *str2, size_t numbytes, int equality) {return 0;}
#else
__attribute__((weak))
...
__attribute__((weak, used))
int AVX_memcmpV1(const void *str1, const void *str2, size_t numbytes, int equality) {return 0;}
#else
__attribute__((weak))
