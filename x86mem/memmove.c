...
//
// This file provides a highly optimized version of memmove_fpx86.
...
// for best performance. For memmove_fpx86, this also means there's no point in doing
...
//
// For pure memmove_fpx86, this provides no real improvement, but loops with many
...
static void * memmove_16bit(void *dest, const void *src, size_t len)
{
...
static void * memmove_32bit(void *dest, const void *src, size_t len)
{
...
static void * memmove_64bit(void *dest, const void *src, size_t len)
{
...
static void * memmove_128bit_u(void *dest, const void *src, size_t len)
{
...
// 32 bytes at a time
static void * memmove_128bit_32B_u(void *dest, const void *src, size_t len)
{
  const __m128i_u* s = (__m128i_u*)src;
  __m128i_u* d = (__m128i_u*)dest;
...
// 64 bytes at a time
static void * memmove_128bit_64B_u(void *dest, const void *src, size_t len)
{
  const __m128i_u* s = (__m128i_u*)src;
  __m128i_u* d = (__m128i_u*)dest;
...
// 128 bytes at a time
static void * memmove_128bit_128B_u(void *dest, const void *src, size_t len)
{
  const __m128i_u* s = (__m128i_u*)src;
  __m128i_u* d = (__m128i_u*)dest;
...
// 256 bytes
static void * memmove_128bit_256B_u(void *dest, const void *src, size_t len)
{
  const __m128i_u* s = (__m128i_u*)src;
  __m128i_u* d = (__m128i_u*)dest;
...
static void * memmove_256bit_u(void *dest, const void *src, size_t len)
{
...
// 64 bytes at a time
static void * memmove_256bit_64B_u(void *dest, const void *src, size_t len)
{
  const __m256i_u* s = (__m256i_u*)src;
  __m256i_u* d = (__m256i_u*)dest;
...
// 128 bytes at a time
static void * memmove_256bit_128B_u(void *dest, const void *src, size_t len)
{
  const __m256i_u* s = (__m256i_u*)src;
  __m256i_u* d = (__m256i_u*)dest;
...
// 256 bytes at a time
static void * memmove_256bit_256B_u(void *dest, const void *src, size_t len)
{
  const __m256i_u* s = (__m256i_u*)src;
  __m256i_u* d = (__m256i_u*)dest;
...
static void * memmove_256bit_512B_u(void *dest, const void *src, size_t len)
{
  const __m256i_u* s = (__m256i_u*)src;
  __m256i_u* d = (__m256i_u*)dest;
...
#ifdef __AVX512F__
static void * memmove_512bit_u(void *dest, const void *src, size_t len)
{
...
// 128 bytes at a time
static void * memmove_512bit_128B_u(void *dest, const void *src, size_t len)
{
  const __m512i_u* s = (__m512i_u*)src;
  __m512i_u* d = (__m512i_u*)dest;
...
// 256 bytes at a time
static void * memmove_512bit_256B_u(void *dest, const void *src, size_t len)
{
  const __m512i_u* s = (__m512i_u*)src;
  __m512i_u* d = (__m512i_u*)dest;
...
static void * memmove_512bit_512B_u(void *dest, const void *src, size_t len)
{
  const __m512i_u* s = (__m512i_u*)src;
  __m512i_u* d = (__m512i_u*)dest;
...
// 1024 bytes, or 1 kB
static void * memmove_512bit_1kB_u(void *dest, const void *src, size_t len)
{
  const __m512i_u* s = (__m512i_u*)src;
  __m512i_u* d = (__m512i_u*)dest;
...
// 2048 bytes, or 2 kB
static void * memmove_512bit_2kB_u(void *dest, const void *src, size_t len)
{
  const __m512i_u* s = (__m512i_u*)src;
  __m512i_u* d = (__m512i_u*)dest;
...
// 4096 bytes, or 4 kB
static void * memmove_512bit_4kB_u(void *dest, const void *src, size_t len)
{
  const __m512i_u* s = (__m512i_u*)src;
  __m512i_u* d = (__m512i_u*)dest;
...
static void * memmove_128bit_a(void *dest, const void *src, size_t len)
{
...
// 32 bytes at a time
static void * memmove_128bit_32B_a(void *dest, const void *src, size_t len)
{
  const __m128i* s = (__m128i*)src;
  __m128i* d = (__m128i*)dest;
...
// 64 bytes at a time
static void * memmove_128bit_64B_a(void *dest, const void *src, size_t len)
{
  const __m128i* s = (__m128i*)src;
  __m128i* d = (__m128i*)dest;
...
// 128 bytes at a time
static void * memmove_128bit_128B_a(void *dest, const void *src, size_t len)
{
  const __m128i* s = (__m128i*)src;
  __m128i* d = (__m128i*)dest;
...
// 256 bytes
static void * memmove_128bit_256B_a(void *dest, const void *src, size_t len)
{
  const __m128i* s = (__m128i*)src;
  __m128i* d = (__m128i*)dest;
...
#ifdef __AVX__
static void * memmove_256bit_a(void *dest, const void *src, size_t len)
{
...
// 64 bytes at a time
static void * memmove_256bit_64B_a(void *dest, const void *src, size_t len)
{
  const __m256i* s = (__m256i*)src;
  __m256i* d = (__m256i*)dest;
...
// 128 bytes at a time
static void * memmove_256bit_128B_a(void *dest, const void *src, size_t len)
{
  const __m256i* s = (__m256i*)src;
  __m256i* d = (__m256i*)dest;
...
// 256 bytes at a time
static void * memmove_256bit_256B_a(void *dest, const void *src, size_t len)
{
  const __m256i* s = (__m256i*)src;
  __m256i* d = (__m256i*)dest;
...
static void * memmove_256bit_512B_a(void *dest, const void *src, size_t len)
{
  const __m256i* s = (__m256i*)src;
  __m256i* d = (__m256i*)dest;
...
#ifdef __AVX512F__
static void * memmove_512bit_a(void *dest, const void *src, size_t len)
{
...
// 128 bytes at a time
static void * memmove_512bit_128B_a(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
// 256 bytes at a time
static void * memmove_512bit_256B_a(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
// 512 bytes (half a KB!!) at a time
static void * memmove_512bit_512B_a(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
// 1024 bytes, or 1 kB
static void * memmove_512bit_1kB_a(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
static void * memmove_512bit_2kB_a(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
// Y'know what? Here's a whole page.
// 4096 bytes, or 4 kB
static void * memmove_512bit_4kB_a(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
static void * memmove_128bit_as(void *dest, const void *src, size_t len)
{
...
// 32 bytes at a time
static void * memmove_128bit_32B_as(void *dest, const void *src, size_t len)
{
  __m128i* s = (__m128i*)src;
  __m128i* d = (__m128i*)dest;
...
// 64 bytes at a time
static void * memmove_128bit_64B_as(void *dest, const void *src, size_t len)
{
  __m128i* s = (__m128i*)src;
  __m128i* d = (__m128i*)dest;
...
// 128 bytes at a time
static void * memmove_128bit_128B_as(void *dest, const void *src, size_t len)
{
  __m128i* s = (__m128i*)src;
  __m128i* d = (__m128i*)dest;
...
// For fun: 1 load->store for every xmm register (there are 16)
// 256 bytes
static void * memmove_128bit_256B_as(void *dest, const void *src, size_t len)
{
  __m128i* s = (__m128i*)src;
  __m128i* d = (__m128i*)dest;
...
#ifdef __AVX2__
static void * memmove_256bit_as(void *dest, const void *src, size_t len)
{
  const __m256i* s = (__m256i*)src;
  __m256i* d = (__m256i*)dest;
...
// 64 bytes at a time
static void * memmove_256bit_64B_as(void *dest, const void *src, size_t len)
{
  const __m256i* s = (__m256i*)src;
  __m256i* d = (__m256i*)dest;
...
// 128 bytes at a time
static void * memmove_256bit_128B_as(void *dest, const void *src, size_t len)
{
  const __m256i* s = (__m256i*)src;
  __m256i* d = (__m256i*)dest;
...
// 256 bytes at a time
static void * memmove_256bit_256B_as(void *dest, const void *src, size_t len)
{
  const __m256i* s = (__m256i*)src;
  __m256i* d = (__m256i*)dest;
...
// I just wanted to see what doing one move for every ymm register looks like.
// There are 16 256-bit (ymm) registers.
static void * memmove_256bit_512B_as(void *dest, const void *src, size_t len)
{
  const __m256i* s = (__m256i*)src;
  __m256i* d = (__m256i*)dest;
...
#ifdef __AVX512F__
static void * memmove_512bit_as(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
// 128 bytes at a time
static void * memmove_512bit_128B_as(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
// 256 bytes at a time
static void * memmove_512bit_256B_as(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
// 512 bytes (half a KB!!) at a time
static void * memmove_512bit_512B_as(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
// 1024 bytes, or 1 kB
static void * memmove_512bit_1kB_as(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
// 2048 bytes, or 2 kB
// AVX512 has 32x 512-bit registers, so......
static void * memmove_512bit_2kB_as(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
// Y'know what? Here's a whole page.
// 4096 bytes, or 4 kB
static void * memmove_512bit_4kB_as(void *dest, const void *src, size_t len)
{
  const __m512i* s = (__m512i*)src;
  __m512i* d = (__m512i*)dest;
...
{
  void * returnval = dest; // memmove_fpx86 is supposed to return the destination
  ...
  // NOTE: Each memmove_fpx86 has its own loop so that any one can be used individually.
  {
    if(numbytes < 2) // 1 byte
    {
      memmove_fpx86(dest, src, numbytes);
      offset = numbytes & -1;
...
static void * memmove_large_reverse(void *dest, void *src, size_t numbytes)
{
  ...
  // NOTE: Each memmove_fpx86 has its own loop so that any one can be used individually.
  {
    if(numbytes & 1) // 1 byte
    {
      ...
      memmove_fpx86(nextdest, nextsrc, 1);
      numbytes &= -2;
    }
...
void *AVX_memmoveV3(void *dest, void *src, size_t numbytes) {return dest;}
#else
__attribute__((weak))
...
__attribute__((weak, used))
void *AVX_memmoveV2(void *dest, void *src, size_t numbytes) {return dest;}
#else
__attribute__((weak))
...
__attribute__((weak, used))
void *AVX_memmoveV1(void *dest, void *src, size_t numbytes) {return dest;}
#else
__attribute__((weak))
