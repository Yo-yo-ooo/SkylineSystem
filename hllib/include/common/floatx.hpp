...
//a copy version of FloatX
//from https://github.com/oprecomp/FloatX/blob/master/src/floatx.hpp
...
#include <stddef.h>
#include <stdint.h>
#include <common/floatxdep.hpp>

#define FLOATX_ATTRIBUTES
#ifdef __GNUC__
#define FLOATX_INLINE inline __attribute__((always_inline))
#else
#define FLOATX_INLINE 
#endif
#define yfloat_assert(x) 
...
namespace flx {
    ...
    namespace detail {
        ...
        static constexpr int32_t bits_in_byte = CHAR_BIT;
...
#define FLOATX_USE_DEFAULT_TRAITS(_type)                                 \
    ...
    struct float_traits<T,
        ...
        static const int32_t exp_bits = 0;
        static const int32_t sig_bits = 0;
    ...
    struct float_traits<float, void> {
        static const bool is_floatx = false;
        static const bool is_runtime = false;
        static const int32_t exp_bits = 8;
        static const int32_t sig_bits = 23;
    ...
    struct float_traits<double, void> {
        static const bool is_floatx = false;
        static const bool is_runtime = false;
        static const int32_t exp_bits = 11;
        static const int32_t sig_bits = 52;
...
#define ENABLE_PROPERTY(_prop)                                              \
    ...
    FLOATX_ATTRIBUTES FLOATX_INLINE constexpr auto get_##_prop(             \
        ...
        typename FLOATX_DEPEND::enable_if<!float_traits<Float>::is_runtime,           \
    ...
    FLOATX_ATTRIBUTES FLOATX_INLINE constexpr auto get_##_prop(             \
        ...
        typename FLOATX_DEPEND::enable_if<float_traits<RuntimeFloat>::is_runtime,     \
...
#undef ENABLE_PROPERTY
    ...
    template <int32_t ExpBits, int32_t SigBits, typename BackendFloat = double>
    class floatx
    ...
    template <int32_t ExpBits, int32_t SigBits, typename BackendFloat>
    struct float_traits<floatx<ExpBits, SigBits, BackendFloat>, void> {
        static const bool is_floatx = true;
        static const bool is_runtime = false;
        static const int32_t exp_bits = ExpBits;
        static const int32_t sig_bits = SigBits;
    ...
    struct float_traits<floatxr<BackendFloat, MetadataType>, void> {
        ...
        static const int32_t exp_bits = float_traits<BackendFloat>::exp_bits;
        static const int32_t sig_bits = float_traits<BackendFloat>::sig_bits;
        using backend_float = BackendFloat;
    ...
    struct supertype {
    private:
        static constexpr int32_t max(int32_t x, int32_t y) { return (x > y) ? x : y; }
    ...
    public:
        ...
        using type = typename FLOATX_DEPEND::enable_if<
            float_traits<FloatX1>::is_floatx || float_traits<FloatX2>::is_floatx,
            typename FLOATX_DEPEND::conditional<float_traits<FloatX1>::is_runtime ||
        ...
        static constexpr int32_t max_exp_bits(FloatX1 x, FloatX2 y)
        {
            return max(get_exp_bits(x), get_exp_bits(y));
        }
        static constexpr int32_t max_sig_bits(FloatX1 x, FloatX2 y)
        {
...
#define ENABLE_RELATIONAL_OPERATOR(_op)                              \
    ...
    FLOATX_ATTRIBUTES FLOATX_INLINE                                  \
        typename FLOATX_DEPEND::enable_if<float_traits<Float1>::is_floatx ||   \
...
#define ENABLE_ARITHMETIC_OPERATOR(_op)                                        \
    template <typename Float1, typename Float2>                                \
    FLOATX_ATTRIBUTES FLOATX_INLINE typename FLOATX_DEPEND::enable_if<                   \
...
#undef ENABLE_ARITHMETIC_OPERATOR
    ...
    FLOATX_INLINE typename enable_if<float_traits<FloatX>::is_floatx,
        std::ostream&>::type&
    ...
    */
    ...
    template <typename Float>
    FLOATX_ATTRIBUTES FLOATX_INLINE
        FLOATX_DEPEND::bitset<sizeof(typename float_traits<Float>::backend_float)>
        bits(const Float& x) noexcept
    {
        using bf = typename float_traits<Float>::backend_float;
        using bitset = bitset<sizeof(bf)>;
        bf val = bf(x);
    ...
    namespace detail {
            ...
            uint16_t SHIFT)
        {
            ...
            yfloat_assert(!(mant & (UINT64_C(0x1) << 63)));
        ...
        const int32_t BACKEND_E = 11;
        const int32_t BACKEND_M = 52;
        ...
        FLOATX_ATTRIBUTES FLOATX_INLINE void convert_subnormal_mantissa_and_exp(
            const uint64_t number, const uint8_t M, const int16_t emin, const int32_t e,
        ...
        FLOATX_ATTRIBUTES FLOATX_INLINE void fix_too_large_mantissa(const int32_t M, int32_t& e,
            uint64_t& mant,
            uint64_t& exp)
        {
            e += 1;
            ...
            const uint64_t sign_pattern, const uint64_t mant, const uint64_t exp)
        {
            ...
            // locations.
            yfloat_assert((mant & ~MASK_MANTISSA) == 0x0);
            yfloat_assert((exp & ~MASK_EXPONENT) == 0x0);
            ...
            get_exponent_zero_or_nan_or_inf_exp(const uint64_t exp, const uint8_t E)
        {
            ...
            // yfloat_assert no bits are set at positions 15:E.
            // information is encoded only at positons E-1:0.
            yfloat_assert(target_exp < (0x1 << E));
            return target_exp;
        }
            ...
            get_exponent_regular_backend_exp(const uint64_t exp, const uint8_t E)
        {
            ...
            int32_t e = (exp >> BACKEND_M) - BACKEND_BIAS;
            ...
            get_mantissa_zero_or_nan_or_inf_exp(const uint64_t mant, const uint8_t M)
        {
            ...
            yfloat_assert(ret < (UINT64_C(0x1) << M));
            return ret;
        }
        ...
        {
            yfloat_assert(exp < (0x1 << E));
            yfloat_assert(mant < (UINT64_C(0x1) << M));
        ...
        // Bitset wrappers.
        template <uint8_t E>
        FLOATX_ATTRIBUTES FLOATX_INLINE FLOATX_DEPEND::bitset<E> get_exponent_from_backend_BS(
            const double d)
        {
            return bitset<E>(get_exponent_from_backend(d, E));
        }

        template <uint8_t E, uint8_t M>
        FLOATX_ATTRIBUTES FLOATX_INLINE FLOATX_DEPEND::bitset<M> get_mantissa_from_backend_BS(
            const double d)
        {
            return bitset<M>(get_mantissa_from_backend(d, E, M));
        }

        template <uint8_t E, uint8_t M>
        FLOATX_ATTRIBUTES FLOATX_INLINE FLOATX_DEPEND::bitset<1 + E + M>
            get_fullbit_representation_BS(const double d)
        {
            return bitset<1 + E + M>(get_fullbit_representation(d, E, M));
        }

        template <uint8_t E, uint8_t M>
        FLOATX_ATTRIBUTES FLOATX_INLINE double construct_number(bool sign,
            FLOATX_DEPEND::bitset<E> exp,
            FLOATX_DEPEND::bitset<M> mant)
        {
        ...
        template <uint8_t E, uint8_t M>
        FLOATX_ATTRIBUTES FLOATX_INLINE double construct_number(
            FLOATX_DEPEND::bitset<1 + E + M> fullbit_representation)
        {
...
#undef ENABLE_EXTRACT_PART
        ...
        private:
                ...
                backend_float enforce_rounding(backend_float value) const noexcept
            {
                ...
                int32_t exp = (raw_exp >> backend_exp_pos) - backend_bias;

                const int32_t emax = (1 << (exp_bits - 1)) - 1;
                const int32_t emin = 1 - emax;
            ...
            static constexpr FLOATX_ATTRIBUTES bool exponent_is_out_of_range(int32_t exp,
                int32_t emax)
            {
                return exp > emax;
            }
    ...
    /* template <typename Float>
    FLOATX_ATTRIBUTES FLOATX_INLINE char* bitstring(const Float& x) noexcept
    {
        using bf = typename float_traits<Float>::backend_float;
        const uint64_t one = UINT64_C(1);
        const char map[] = { '0', '1' };
        const int32_t eb = get_exp_bits(x);
        const int32_t sb = get_sig_bits(x);
        const int32_t beb = get_exp_bits(bf(x));
        const int32_t bsb = get_sig_bits(bf(x));

        //std::string s(sb + eb + 3, '-');
        char* s = (char*)kmalloc(sb + eb + 3);
